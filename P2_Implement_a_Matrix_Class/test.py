...
def test():
    ...
    assert (4*m.identity(5)).matrix[0][0] == 4, "Error in your __rmul__ function"
...
def equal(m1, m2):
    #m1 = m.Matrix([m1])
    #m2 = m.Matrix([m2])
    if len(m1.matrix) != len(m2.matrix): return False
    if len(m1.matrix[0]) != len(m2.matrix[0]): return False
    for r1, r2 in zip(m1.matrix, m2.matrix):
